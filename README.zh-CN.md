...
- 统一 AI 服务层，润色、语法检查、PDF 简历导入共用 Provider 适配，覆盖 Doubao、DeepSeek、OpenAI、Gemini。
- 增加 AI JSON 结构化解析与 normalize，语法错误和导入简历数据都有统一校验入口。
- OpenAI 增加兼容模式，可关闭 `reasoning_effort`，适配第三方 OpenAI-compatible 网关。
- 清理遗留 Next 风格 API，统一使用 TanStack Start 的 `src/routes/api/*`。
- 补充 `pnpm test` 和 `pnpm typecheck`，恢复测试与类型健康检查。
- 移除 `DEFAULT_TEMPLATES` 循环 re-export，消除 Vite/Rollup circular chunk 警告。
- 新增统一富文本 sanitizer，覆盖简历 HTML 渲染和导入落库链路。
- 清理 Next 迁移残留，并补充 File System Access API 局部类型声明。
- 新增本地优先的 JD 岗位匹配流程，支持匹配分、关键词覆盖和可逐条应用的 AI 改写建议。
